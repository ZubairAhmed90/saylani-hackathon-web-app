...
import { Calendar, Clock, Users, Trophy, Code, CheckCircle2 } from "lucide-react"
      ...
      try {
        ...
        const enrichedEnrollments = await Promise.all(
          enrollmentsData.map(async (enrollment) => {
            ...
            const hackathonDoc = await getDocs(
              query(collection(db, "hackathons"), where("__name__", "==", enrollment.hackathonId)),
            )
            ...
            if (enrollment.teamId) {
              const teamDoc = await getDocs(query(collection(db, "teams"), where("__name__", "==", enrollment.teamId)))
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-[calc(100vh-64px)]">
        <div className="animate-pulse text-xl">Loading your hackathons...</div>
      </div>
    )
  }
                  ...
                  <CardFooter>
                    {enrollment.status === "active" ? (
                      ...
                      <Button asChild variant="outline" className="border-gray-700 hover:bg-gray-800">
                        <Link href={`/hackathons/${enrollment.hackathonId}`}>View Results</Link>
                      </Button>
                    ) : (
                      <Button asChild variant="outline" className="border-gray-700 hover:bg-gray-800">
