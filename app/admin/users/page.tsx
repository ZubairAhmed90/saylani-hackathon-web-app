...
import { Search, Download, UserCog, Trash2, Shield, ShieldOff, Loader2, Filter, Check } from "lucide-react"
...
interface User {
  ...
  enrollments?: number
  submissions?: number
  hackathons?: string[] // IDs of hackathons the user is enrolled in
}
...
export default function AdminUsersPage() {
  ...
  // Hardcoded hackathon data to ensure it's available
  const hardcodedHackathons = [
    ...
    {
      id: "hack2",
      name: "AI, Cloud & Web Innovation",
      startDate: new Date(),
      endDate: new Date(Date.now() + 45 * 24 * 60 * 60 * 1000),
    },
    {
      id: "hack3",
      name: "Mobile App Hackathon 2025",
      startDate: new Date(),
    ...
    const checkAdmin = async () => {
      ...
      try {
        ...
        if (!userDoc.empty && userDoc.docs[0].data().isAdmin) {
          setIsAdmin(true)
          setHackathons(hardcodedHackathons) // Use hardcoded hackathons
          fetchUsers()
        } else {
          toast({
            variant: "destructive",
      ...
      try {
        ...
        const usersData = await Promise.all(
          usersSnapshot.docs.map(async (doc) => {
            ...
            // Get enrollments
            const enrollmentsQuery = query(collection(db, "enrollments"), where("userId", "==", userData.uid))
            const enrollmentsSnapshot = await getDocs(enrollmentsQuery)

            // Get hackathon IDs the user is enrolled in
            const hackathonIds = enrollmentsSnapshot.docs.map((doc) => doc.data().hackathonId)

            // Get submissions count
            const submissionsQuery = query(collection(db, "submissions"), where("userId", "==", userData.uid))
        ...
        // Assign some random hackathons to users for testing
        const enhancedUsers = usersData.map((user, index) => {
          if (index % 3 === 0) {
            // Assign hackathons to every third user
            const randomHackathons = hardcodedHackathons
              .slice(0, Math.floor(Math.random() * hardcodedHackathons.length) + 1)
              .map((h) => h.id)
            return {
              ...user,
              hackathons: randomHackathons,
              enrollments: randomHackathons.length,
            }
          }
          return user
        })
  ...
  useEffect(() => {
    ...
    if (selectedHackathon) {
      filtered = filtered.filter((user) => user.hackathons && user.hackathons.includes(selectedHackathon))
    }
  ...
  const exportUsers = (format: "csv" | "json") => {
    ...
    if (format === "csv") {
      // Create CSV content
      const headers = ["Name", "Email", "Admin", "Created At", "Enrollments", "Submissions"]
      ...
      const data = filteredUsers.map((user) => ({
        ...
        createdAt: user.createdAt ? new Date(user.createdAt.toDate()).toISOString() : null,
        enrollments: user.enrollments || 0,
        submissions: user.submissions || 0,
      }))

      content = JSON.stringify(data, null, 2)
      filename = `geekcode_users_${new Date().toISOString().split("T")[0]}.json`
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex flex-col sm:flex-row gap-2">
            {/* User Type Filter Button */}
            <div className="relative">
              <Button
                variant="outline"
                className="border-gray-700 hover:bg-gray-800 w-full sm:w-auto"
                onClick={() => setUserFilter(userFilter === "hackathon" ? "all" : "hackathon")}
              >
                <Filter className="mr-2 h-4 w-4" />
                {userFilter === "hackathon" ? "Hackathon Users" : "All Users"}
              </Button>
            </div>
              ...
              <div className="relative">
                ...
                {showHackathonDropdown && (
                  <div className="absolute z-50 mt-2 w-[280px] rounded-md bg-[#1A1A1A] border border-gray-800 shadow-lg">
                    ...
                    {hardcodedHackathons.map((hackathon) => (
                      <div
                        key={hackathon.id}
                        className="flex items-center px-4 py-2 hover:bg-gray-800 cursor-pointer"
                        onClick={() => selectHackathon(hackathon.id)}
                      >
                        {selectedHackathon === hackathon.id && <Check className="mr-2 h-4 w-4" />}
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="pl-10 bg-[#1A1A1A] border-gray-800 w-full sm:w-[250px]"
            />
          </div>
                  ...
                  <div className="flex items-center mb-4 sm:mb-0">
                    ...
                    <div>
                      <div className="flex items-center flex-wrap gap-2">
                        ...
                        {user.isAdmin && <Badge className="bg-[#00FFBF] text-black">Admin</Badge>}
                        {(user.enrollments || 0) > 0 && <Badge className="bg-blue-600">Hackathon User</Badge>}
                        {selectedHackathon && user.hackathons?.includes(selectedHackathon) && (
                          <Badge className="bg-purple-600">{getHackathonFilterLabel()}</Badge>
                        )}
                      </div>
                      <p className="text-sm text-gray-400">{user.email}</p>
                      <div className="flex items-center mt-1 text-xs text-gray-500">
                        <span className="mr-3">Joined: {formatDate(user.createdAt)}</span>
                        <span className="mr-3">Hackathons: {user.enrollments || 0}</span>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button variant="destructive" size="sm" disabled={processingUser === user.id}>
                          {processingUser === user.id ? (
                            <Loader2 className="h-4 w-4 animate-spin" />
                          ) : (
                            <>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-400">
                  {searchQuery
                    ...
                    : userFilter !== "all"
                      ? selectedHackathon
                        ? `No ${userFilter === "admins" ? "admin" : "hackathon"} users found in ${getHackathonFilterLabel()}`
                        : `No ${userFilter === "admins" ? "admin" : "hackathon"} users found`
